...
처음에는 친구들과 쓸 Discord 봇이니 명령 몇 개만 만들면 된다고 생각했습니다.
그런데 “게임 중 방송을 안 하면 몰랭”처럼 사람끼리는 대충 통하는 규칙도 코드로
옮기려면 게임 시작 시점, 방송 유예시간, 오래된 관측값과 API 실패를 어디까지
믿을지 하나씩 정해야 했습니다. 이 과정을 거치며 모호한 아이디어를 화면과 기능으로
바로 옮기기보다, 먼저 상태와 예외, 완료 기준으로 풀어내는 습관을 얻었습니다.

또 AI를 많이 사용하는 것과 AI에게 맡겨 버리는 것은 전혀 다르다는 것도
배웠습니다. 문제와 승인 범위는 제가 정하고, AI가 만든 코드와 판단은 diff,
테스트와 실제 환경에서 다시 확인해야 했습니다. 결국 Agent를 만드는 것보다
Agent와 함께 만든 결과를 믿고 운영할 수 있게 만드는 일이 더 어려웠습니다.

### 아쉬웠던 점

재미있는 아이디어가 생길 때마다 요약, Riot 관측, 대시보드, KBO 기능을 계속
붙이다 보니 어느 순간 프로젝트의 범위가 너무 커졌습니다. 안전하게 만들겠다는
마음으로 문서와 검증 절차도 촘촘하게 쌓았지만, 정작 친구들이 자주 쓰는 핵심
흐름을 더 빨리 보여 주고 반응을 받는 데에는 늦었습니다. 다시 만든다면 기능 하나를
실제 사용자에게 먼저 끝까지 전달하고, 반복해서 나타나는 문제만 다음 기능으로
확장할 것 같습니다.

### 좋았던 점

가장 좋았던 순간은 코드가 한 번에 맞았을 때보다, 예상하지 못한 실패를 다음
결정의 근거로 바꿨을 때였습니다. 분리 배포 실험이 계속 실패하자 한 서버 구조로
단순화했고, Discord 자료구조를 잘못 흉내 낸 테스트나 배포 직후의 readiness
경쟁 조건은 실제와 가까운 회귀 테스트로 남겼습니다. 덕분에 React 화면부터 API,
Discord Gateway, PostgreSQL, CI/CD와 서버 운영까지 한 제품의 전체 흐름을 직접
이어 볼 수 있었습니다. 무엇보다 개인 프로젝트도 “내 컴퓨터에서 동작함”을 넘어
실제로 배포하고, 실패를 관찰하고, 다시 고치는 데까지 해 본 경험이 즐거웠습니다.
...
| `/크보 베팅 가입 동의:true` | 비현금성 크레딧·공개 랭킹·보존 정책에 동의하고 KBO 기능 가입 |
| `/크보 크레딧 내정보` | 내 가용 크레딧과 정정 부채 조회 |
| `/크보 크레딧 받기` | KST 날짜별 50,000 크레딧 직접 지급 |
| `/크보 베팅 경기` | 최신 접수 가능 경기와 베팅용 경기 ID 조회 |
| `/크보 베팅 하기 경기:<ID> 결과:<결과> 금액:<1,000~50,000>` | 시작 전 KBO 경기 결과 또는 정확 점수 예측 |
| `/크보 베팅 내역` | 최근 베팅 5개와 정산·무효·정정 결과 조회 |
| `/크보 랭킹 크레딧` | 현재 보유 크레딧 공동 순위 조회 |
| `/크보 랭킹 결과\|점수\|적중률 대회:<ID> 시즌:<ID>` | 대회·시즌별 승부 예측 공동 순위 조회 |
