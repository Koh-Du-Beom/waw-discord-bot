...
D-03 데이터 흐름·위협 모델과 D-08 배포 경계·내부 통신 조사 완료, D-08 검토 준비
...
- D-08 다중 관점 검토 완료, `D08-Q01`~`D08-Q06` 소유자 결정 대기
- 예상 사용량과 고정비가 미확정인 항목의 복수 시나리오 유지

## 다음 작업

`docs/research/technology-options/deployment-boundary-and-internal-communication.md`의
`D08-Q01`~`D08-Q06`을 소유자에게 하나씩 확인합니다.
결정 뒤에도 D-05 저장소와 D-07 인증 연구를 거쳐야 D-08 경계를 선택할 수 있으며,
아직 ADR이나 Spike를 작성하지 않습니다.
...
- Vercel 유료 plan, GPT API, 도메인, 외부 백업을 합친 원화 비용이 월 3만 원을 충족하는지 미확정
- MacBook의 절전·재부팅·회선 장애와 물리 접근 조건에서 RPO 24시간·RTO 8시간을 달성할 수 있는지 미검증
- 정책의 배포 형태 1(단일 지속 서버)과 형태 4(전체 자체 호스팅)의 물리 범위 구분이 미확정
- 비동기 설정 적용의 허용 여부·최대 지연·멱등 결과 보존 기간과 RPO/RTO 판정 범위가 미확정
