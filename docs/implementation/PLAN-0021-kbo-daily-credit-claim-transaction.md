...
- Depends on: migrations `0013`·`0014`, completed `PLAN-0019`·`PLAN-0020`
- Owner: Product owner
...
- 롤백: Discord/runtime에 연결되지 않은 새 store와 domain 파일만 제거하며
  migrations `0013`·`0014`와 Production 데이터에는 영향이 없다.
...
검증하고 Discord command나 bot runtime에 연결하지 않는다. Migrations `0013`·
`0014`의 Production 적용은 exact owner gate와 backup·restore 증거 전까지 금지한다.
