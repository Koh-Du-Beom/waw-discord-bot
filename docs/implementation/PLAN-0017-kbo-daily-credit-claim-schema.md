...
- Depends on: `PLAN-0015` migration `0011`, `PLAN-0016` enrollment transaction
- Owner: Product owner
...
- Migration `0011`은 0잔액 `credit_account`, active `betting_enrollment`와
...
- Additive migration `0012`의 `daily_credit_claim` table
...
- 목적: 일일 지급 구현 전에 날짜별 중복 방지와 claim-ledger 참조 무결성만
  PostgreSQL에 고정한다.
- 변경 예상 파일: `migrations/0012_kbo_daily_credit_claim.sql`,
...
- Owner decision: Approved — Task 1의 additive `0012` migration과 PostgreSQL
  계약 테스트만 구현; Production 적용 금지
- Approved date: 2026-08-07

## 진행 결과

- Additive `0012` migration으로 `daily_credit_claim`과 schema version `12`를
  등록했다.
