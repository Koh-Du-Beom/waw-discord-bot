...
첫 MVP의 신규 월 지출 상한은 GPT API와 필수 외부 백업·도메인 부대비용을 포함해 3만 원이다. 기존 MacBook 상시 실행은 우선 조사 시나리오일 뿐 확정된 호스트가 아니다. D-08 배포 경계는 `ADR-0001`로 Accepted 되었고 D-04 런타임·SDK 선택은 보류 중이다. 서울 Lightsail 1GB fixture의 Node·Python 60분 합성 수용량 verifier는 통과했지만 실제 Gateway credential, disconnect/Resume과 Go Live reconciliation은 검증하지 않았다.
