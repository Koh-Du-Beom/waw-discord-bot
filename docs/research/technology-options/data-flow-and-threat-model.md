...
- 같은 브라우저의 로그인은 마지막 활동 후 7일, 최초 로그인 후 최대 30일까지 유지한다. 로그아웃·guild 탈퇴·role 상실 시 즉시 폐기하고 고위험 작업 전 현재 role을 재확인한다.
...
| 브라우저 → 대시보드 | OAuth callback, 쿠키, 설정 변경·조회 | 불신 | state·CSRF·세션 고정 방지, 서버 측 인가, 7일 비활동·30일 절대 만료, 고위험 작업 역할 재조회, 감사 |
...
| OAuth·세션 비밀 | access/refresh token, session ID·서명 material | 필요한 최소 기간만 보호 저장 가능; 브라우저에는 임의 세션 식별자만 저장 | 7일 비활동 또는 30일 절대 만료; 로그아웃·guild 탈퇴·role 상실 시 즉시 폐기 또는 revoke | 로그·감사·URL·웹 화면 노출·평문 backup 금지 |
...
2. callback에서 state와 token 응답을 검증하고 Discord user ID를 얻는다.
3. 허용된 단일 guild의 현재 member/roles를 서버에서 확인한다.
...
6. 브라우저에는 임의 세션 식별자만 두고, 마지막 활동 후 7일 또는 최초 로그인 후 30일 중 먼저 도달한 시점에 세션을 만료한다. 로그아웃·guild 탈퇴·role 상실 시 즉시 폐기한다.
7. 보호 요청마다 서버 측 권한을 검사한다. 역할 cache 방식과 일반 요청 재검증 주기는 D-07에서 비교한다.
8. 권한·복구 관련 고위험 작업 직전에는 현재 guild member/roles를 강제 재조회한다. 조회 실패는 기본 거부한다.
...
|---|---|---|---|---|
| 다른 guild 또는 비승인 사용자 접근 | 위조·오래된 세션, callback 조작, guild 탈퇴 뒤 기존 권한 사용 | 대시보드·설정 무단 접근 | 허용 guild 고정, OAuth state, 서버 측 role 검사, 권한 상실 시 폐기, 기본 거부 | 일반 요청의 role cache/재검증 주기는 D-07 |
...
| 일반 요청의 role 재검증 주기와 세션 구조 | 7일 비활동·30일 절대 만료, 고위험 작업 강제 재조회, 나머지는 기본 거부 원칙 | D-07 인증·인가·세션 연구 |
