...
7. 보호 요청마다 서버 측 권한을 검사한다. 최대 5분의 유효한 역할 cache에서는 read-only만 허용하고, Discord 재조회 실패 시 변경을 거부한다. cache가 5분을 넘으면 조회도 `unavailable`로 처리한다.
8. 권한·복구 관련 고위험 작업은 15분 이내 로그인, 현재 guild member/roles 강제 재조회, CSRF 검사와 명시적 사용자 확인을 모두 통과해야 한다. 하나라도 실패하면 거부한다.
