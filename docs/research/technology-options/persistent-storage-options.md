...
- Supabase Free 후보는 owner 승인 후 별도 disposable project(`waw-storage-spike-20260721`)를 생성했다. 기존 production 프로젝트는 사용하지 않았다. Orca 내장 브라우저의 새 query에서 RLS를 활성화한 합성 SQL을 실행해 10,000행·10,000 unique operation·관계 크기 1,864 kB를 확인했다. 검증 직후 프로젝트를 삭제했고 Supabase toast로 삭제 성공을 확인했다.
